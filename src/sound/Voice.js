...
export class Voice extends events.EventEmitter {
	constructor() {
		...
		trackConfig.forEach((track) => {
			if (track.intro) {
				this._players.add(track.artist, `${voFolder}/${track.intro}.[ogg|mp3]`);
			}
		});

		this._players.add("experience", `${voFolder}/intro.[ogg|mp3]`);
		this._players.add("loading", `${voFolder}/loading.[ogg|mp3]`);
