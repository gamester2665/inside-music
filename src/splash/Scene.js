	...
	constructor(canvas) {
		...
		this.loop = animitter(function (delta, elapsed, frameCount) {
			...
			const nextAvg = copyAndGetAverage(
				channelData,
				wf,
				(channelDataOffset += 128) % channelData.length,
				128
			);
			...
			const screenSize = clamp(
				(window.innerWidth * window.innerHeight) / 1764000,
				0.5,
				3
			);
		...
		this.__removeListeners = ((self) => {
			...
			const ramp = (elapsed, duration, minOut = 0, maxOut = 0.005) =>
				clamp(map(elapsed, 0, duration, minOut, maxOut), minOut, maxOut);

			this.loop.on("update", (delta, elapsed) => {
				const diff = (goal - rings[0].rotation.x) * ramp(elapsed, 10000);
				rings.forEach((r) => (r.rotation.x += diff));
			});
